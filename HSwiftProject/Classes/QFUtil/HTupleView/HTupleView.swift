...
class HTupleAppearance : NSObject {
    ...
    private static func addTuple(_ anTuple: AnyObject) -> Void {
        self.hashTuples.add(anTuple)
    }
    static func enumerateTuples(_ completion: () -> Void) -> Void {
        DispatchQueue.main.async {
            //倒序执行
            for tuple: HTupleView in self.hashTuples.allObjects.reversed() {
                tuple.reloadData()
            }
            completion()
        }
...
class HTupleView : UICollectionView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HTupleViewDelegate, HCollectionViewDelegateFlowLayout {
    ...
    private var sectionPaths: NSArray<NSNumber>?
    ...
    override convenience init(frame: CGRect) {
        flowLayout = HCollectionViewFlowLayout()
        flowLayout.scrollDirection = .vertical
        self.init(frame, collectionViewLayout: flowLayout)
    }
    ...
    private convenience init(_ frame: CGRect, exclusiveSections sectionPaths: NSArray) {
        flowLayout = HCollectionViewFlowLayout()
        flowLayout.scrollDirection = .vertical
        self.init(UIRectIntegral(frame), collectionViewLayout: flowLayout)
        ...
        set {
            if _releaseTupleKey != newValue {
                if _releaseTupleKey != nil && newValue != nil {
                    NotificationCenter.default.removeObserver(self, name: _releaseTupleKey, object: nil)
                }
                _releaseTupleKey = newValue
                if _releaseTupleKey != nil {
                    NotificationCenter.default.addObserver(self, selector: #selector(releaseTupleBlock), name: _releaseTupleKey, object: nil)
                }
            }
        }
    }
        ...
        set {
            if _reloadTupleKey != newValue {
                if _reloadTupleKey != nil && reloadTupleKey != nil {
                    NotificationCenter.default.removeObserver(self, name: _reloadTupleKey, object: nil)
                }
                _reloadTupleKey = newValue
                if _reloadTupleKey != nil {
                    NotificationCenter.default.addObserver(self, selector: #selector(reloadTupleData), name: _reloadTupleKey, object: nil)
                }
            }
        }
    }
...
//        [self.mj_footer endRefreshingWithCompletionBlock:completion];
    ...
    /// register class
    func dequeueReusableHeaderWithClass(_ cls: AnyClass?, pre: String?, idx: Bool, idxPath: NSIndexPath) -> AnyObject {
        var cell: HTupleBaseApex
        let identifier: String = NSStringFromClass(cls)
        ...
        if pre { identifier = identifier+pre }
        if idx { identifier = identifier+idxPath.getStringValue }
        if self.allReuseIdentifiers.contains(identifier) == false {
            ...
            cell = self.dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionHeader, withReuseIdentifier: identifier, for: idxPath)
            cell.tuple = self
            cell.indexPath = idxPath
            cell.isHeader = true
            //init method
            if iblk != nil {
                let initHeaderBlock: HTupleCellInitBlock = iblk
        ...
        let selector: Selector = #selector(edgeInsetsForHeaderInSection(_:))
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        if delegateObjc.responds(to: selector, withPre: prefix) {
            edgeInsets = delegateObjc.perform(selector, with: idxPath.section)
        }
        //设置属性
        if cell.responds(to: #selector(edgeInsets)) {
            cell.edgeInsets = edgeInsets
        }
        return cell
    }
    
    func dequeueReusableFooterWithClass(_ cls: AnyClass, pre: String, idx: Bool, idxPath: NSIndexPath) -> AnyObject {
        var cell: HTupleBaseApex
        let identifier: String = NSStringFromClass(cls)
        identifier = identifier+self.addressValue
        ...
        if self.allReuseIdentifiers.contains(identifier) == false {
            ...
            cell = self.dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionFooter, withReuseIdentifier: identifier, for: idxPath)
            cell.tuple = self
            cell.indexPath = idxPath
            cell.isHeader = true
            //init method
            if (iblk) {
                let initFooterBlock: HTupleCellInitBlock = iblk
        ...
        let selector = #selector(edgeInsetsForFooterInSection(_:))
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        if delegateObjc.responds(to: selector, withPre: prefix) {
            edgeInsets = delegateObjc.perform(selector, with: idxPath.section)
        }
    ...
    func dequeueReusableCellWithClass(_ cls: AnyClass, pre: String, idx: Bool, idxPath: NSIndexPath) -> AnyObject {
        ...
        if self.allReuseIdentifiers.contains(identifier) == false {
            ...
            cell = self.dequeueReusableCell(withReuseIdentifier: identifier, for: idxPath)
            cell.tuple = self
            cell.indexPath = idxPath
            //init method
            if (iblk) {
                let initCellBlock: HTupleCellInitBlock = iblk
                initCellBlock(cell)
            }
        }else {
            cell = self.dequeueReusableCell(withReuseIdentifier: identifier, for: idxPath)
        ...
        let edgeInsets: UIEdgeInsets = UIEdgeInsetsZero
        let prefix = self.prefixWithSection(idxPath.section)
        let selector = #selector(edgeInsetsForItemAtIndexPath(_:))
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        if delegateObjc.responds(to: selector, withPre: prefix) {
            edgeInsets = delegateObjc.perform(selector, with: idxPath.section)
        }
    ...
    private func numberOfSections(in collectionView: UICollectionView) -> Int {
        ...
        case .default:
            var sections = 0
            let prefix = ""
            let selector = #selector(numberOfSectionsInTupleView())
            let delegateObjc: NSObject = self.tupleDelegate as NSObject
            if delegateObjc.responds(to: selector, withPre: prefix) {
                sections = delegateObjc.perform(selector, with: collectionView)
            }
            return sections
        case .split:
            var sections = 0
    ...
    private func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(numberOfItemsInSection(_:numberOfItemsInSection:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            items = delegateObjc.perform(selector, with: collectionView, with: section)
        }
        let edgeInsets = self.collectionView(self, layout: self.flowLayout, insetForSectionAt: section)
        self.allSectionInsets.setObject(NSStringFromUIEdgeInsets(edgeInsets), forKey: "\(section)")
        return items
    }

    ///layout == HCollectionViewFlowLayout
    private func collectionView(_ collectionView: UICollectionView, layout: UICollectionViewLayout, colorForSectionAtIndex: NSInteger) -> UIColor {
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(colorForSectionAtIndex(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            return delegateObjc.perform(selector, with: section)
        }
    ...
    private func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(insetForSection(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            return delegateObjc.perform(selector, with: section)
        }
    ...
    private func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        ...
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(sizeForHeaderInSection(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            size = delegateObjc.perform(selector, with: section)
        }
    ...
    private func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForFooterInSection section: Int) -> CGSize {
        ...
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(sizeForFooterInSection(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            size = delegateObjc.perform(selector, with: section)
        }
        return UISizeIntegral(size)
    ...
    private func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(sizeForItemAtIndexPath(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
            size = delegateObjc.perform(selector, with: section)
        }
        //不能为CGSizeZero，否则会崩溃
    ...
    @available(iOS 6.0, *)
    private func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {Z
        //调用代理方法
        let prefix: String = self.prefixWithSection(indexPath.section)
        let selector: Selector = #selector(tupleForItem(_:inSection:))
        ...
        //调用cell
        let cell: HTupleBaseCell = self.allReuseCells.object(forKey: indexPath.stringValue as NSString)
        //更新布局
        if cell.responds(to: #selector(relayoutSubviews)) {
    ...
    private func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionElementKindSectionHeader {
            //调用代理方法
            let prefix: String = self.prefixWithSection(indexPath.section)
            let selector: Selector = #selector(tupleForHeader(_:inSection:))
            ...
            //调用cell
            cell = self.allReuseHeaders.object(forKey: indexPath.stringValue as NSString)
        }else if (kind == UICollectionElementKindSectionFooter) {
            //调用代理方法
            let prefix: String = self.prefixWithSection(indexPath.section)
            let selector: Selector = #selector(tupleForFooter(_:inSection:))
            ...
            //调用cell
            cell = self.allReuseFooters.object(forKey: indexPath.stringValue as NSString)
        }
        //更新布局
        if cell.responds(to: #selector(relayoutSubviews)) {
            cell.relayoutSubviews()
    ...
    private func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(willDisplayCell(_:atIndexPath:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
    ...
    private func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let prefix = self.prefixWithSection(section)
        let delegateObjc: NSObject = self.tupleDelegate as NSObject
        let selector = #selector(didSelectItemAtIndexPath(_:))
        if delegateObjc.responds(to: selector, withPre: prefix) {
...
extension HTupleView {
    ...
    var signalBlock: HTupleCellSignalBlock? {
        get {
            return self.getAssociatedValueForKey(#function)
        }
        set {
    ...
    func signalToAllItems(_ signal: HTupleSignal) {
        DispatchQueue.main.async {
            for cell: HTupleBaseCell in self.allReuseCells {
    ...
    func signal(_ signal: HTupleSignal, itemSection section: Int) {
        DispatchQueue.main.async {
            ...
            for i in 0..<items {
                let cell: HTupleBaseCell = self.allReuseCells.object(forKey: NSIndexPath.stringValue(i, section))
                if cell.signalBlock != nil {
                    cell.signalBlock(cell, signal)
                }
            }
    ...
    func signal(_ signal: HTupleSignal, indexPath idxPath: NSIndexPath) {
        let cell: HTupleBaseCell = self.allReuseCells.object(forKey: idxPath.stringValue)
        if cell.signalBlock != nil {
            DispatchQueue.main.async {
                cell.signalBlock(cell, signal)
            }
    ...
    func signalToAllHeader(_ signal: HTupleSignal) {
        DispatchQueue.main.async {
            ...
            for i in 0..<sections {
                let header: HTupleBaseApex = self.allReuseCells.object(forKey: NSIndexPath.stringValue(0, i))
    ...
    func signal(_ signal: HTupleSignal, headerSection section: Int) {
        let header: HTupleBaseApex = self.allReuseCells.object(forKey: NSIndexPath.stringValue(0, section))
        if header.signalBlock != nil {
            DispatchQueue.main.async {
                header.signalBlock(header, signal)
            }
    ...
    func signalToAllFooter(_ signal: HTupleSignal) {
        DispatchQueue.main.async {
            let sections = self.numberOfSections
            for i in 0..<sections {
                let footer: HTupleBaseApex = self.allReuseCells.object(forKey: NSIndexPath.stringValue(0, i))
    ...
    func signal(_ signal: HTupleSignal, footerSection section: Int) {
        let footer: HTupleBaseApex = self.allReuseCells.object(forKey: NSIndexPath.stringValue(0, section))
        if footer.signalBlock != nil {
            DispatchQueue.main.async {
                footer.signalBlock(footer, signal)
            }
    ...
    func releaseAllSignal() {
        DispatchQueue.global().async {
            ...
            //release all header
            for header: HTupleBaseApex in self.allReuseHeaders {
                if header.signalBlock != nil {
                    header.signalBlock = nil
                }
            }
            //release all footer
            for footer: HTupleBaseApex in self.allReuseFooters {
                if footer.signalBlock != nil {
    ...
    func widthWithSection(_ section: Int) -> CGFloat {
        ...
        let edgeInsetsString: String = self.allSectionInsets.object(forKey: "\(section)")
    ...
    func fixSlitWith(_ width: CGFloat, colCount: Int, index: Int) -> CGFloat {
        let itemWidth: CGFloat = width/colCount
        let realItemWidth: CGFloat = floorf(itemWidth)
...
extension HTupleView {
    ...
    private var tupleStateSource: NSMutableDictionary {
        get {
            var dict: NSMutableDictionary? = self.getAssociatedValueForKey(#function)
            if dict == nil {
    ...
    func setObject(_ anObject: AnyObject, forKey aKey: String, state tupleState: HTupleState) -> Void {
        let key = aKey+KTupleStateKey+"\(tupleState)"
    ...
    func objectForKey(_ aKey: String, state tupleState: HTupleState) -> AnyObject {
        let key = aKey+KTupleStateKey+"\(tupleState)"
    ...
    func removeObjectForKey(_ aKey: String) -> Void {
        self.removeObjectForKey(aKey, state: self.tupleStateSource)
    }
    ...
    func removeObjectForState(_ tupleState: HTupleState) -> Void {
        ...
        for aKey in self.tupleStateSource.reversed() {
            if key == aKey {
